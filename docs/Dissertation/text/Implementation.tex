...
\begin{itemize}
	...
	\item Changing the \texttt{pitch} of the map (increasing the pitch flatten the map which allows the user to easily see obsteubed units).
	\item A log which can be used which actions have taken place. Useful to find out what the enemy has done if the user miss it the first time.
	...
	\item The unit information shows the attributes include the current hit points. The menu are displayed in different colour for opposing units to easily discadiish them. 
\end{itemize}
