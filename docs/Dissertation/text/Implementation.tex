...
The visual map editor support editing  of both textured and non-textured tiles. As discussed before non-textured tiles allow the user more control over whats drawn so they  apply more detail. The advantage of textured tiles is that slanted tiles can be used, which easily allow the creation of slopes and stairs for example.
...
\end{figure}
The editor supports the creation of dialog.  A dialog is sequential of parts. Each part has the text  associated with it as well optionally having a speaker.  The speaker can be selected from any unit that has already been placed on the map. This a vast improvement on the original design where the user types the name of the speaker since there no less change of entering invalid data.
...
\begin{itemize}
	...
	\item Creating the the jar of the engine.
