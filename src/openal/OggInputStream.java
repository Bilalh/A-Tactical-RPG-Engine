...
/**
 * An input stream that can extract ogg data. This class is a bit of an experiment with
 * continuations so uses thread where possibly not required. It's just a test to see if
...
public class OggInputStream extends InputStream implements AudioInputStream {
	...
	private byte[] convbuffer = new byte[convsize]; // take 8k out of the data segment, not the
													// stack
	/** The stream we're reading the OGG file from */
	private InputStream input;
	/** The audio information from the OGG header */
	private Info oggInfo = new Info(); // struct that stores all the static vorbis bitstream
										// settings
	...
	private StreamState streamState = new StreamState(); // take physical pages, weld into a logical
															// stream of packets
